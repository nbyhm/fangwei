...
import com.dowell.util.constant.FwConstants;
...
/**
 * @author nanbo
 * @description Apache POI SXSS相关的简易封装
 * @create 2018-10-03
 **/
public class PoiUtils {
	...
	static void writeByLocalOrBrowser(HttpServletResponse response, String fileName, SXSSFWorkbook wb, OutputStream out) {
		try {
			...
			if (response != null) {
				...
				response.setContentType(FwConstants.XLSX_CONTENT_TYPE);
				response.setHeader("Content-disposition", "attachment; filename="
						+ URLEncoder.encode(String.format("%s%s", fileName, FwConstants.XLSX_SUFFIX), "UTF-8"));
				if (out == null) {
	...
	private static void checkExcelFile(String file) {
		if (!file.endsWith(FwConstants.XLSX_SUFFIX)) {
